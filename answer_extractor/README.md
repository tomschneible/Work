...
**Status: setup/auth plumbing only so far -- filling a template in with
real results and exporting it isn't wired up yet.** This section covers
what's implemented today.

Instead of (or alongside) a plain `.xlsx`, results can be written into a
...
1. Create a project at [console.cloud.google.com](https://console.cloud.google.com/)
   (free -- no billing/credit card needed for this).
2. **APIs & Services -> Library**: enable the **Google Sheets API** and
   the **Google Drive API**.
3. **APIs & Services -> OAuth consent screen**: User Type **External**;
   fill in the required fields; add your own Google account under **Test
   users**. This keeps the app in "Testing" status, which is enough to
   sign in as yourself with no Google review process -- the tradeoff is
   that a test user's refresh token expires after 7 days, so you'll get a
   one-click re-consent prompt weekly. Move the app to "In production"
   (still no review needed for a personal tool with this few users, and
   still doesn't publish it anywhere) if you'd rather not deal with that.
...
(or `ANSWER_EXTRACTOR_GOOGLE_TOKEN_CACHE`). Every call after that is
silent unless the token needs re-consent (see the 7-day note above).
**To switch which Google account this points at** (e.g. moving from a
personal account to a different one later), just delete that cached
token file -- the next call re-prompts for consent, with no changes to
the Cloud project or OAuth client needed.
...
Shared Drive too, as long as your account has at least Viewer access to
it) with its own file id and name -- what you need to point the (still
unbuilt) export step at a specific template.

This path is implemented in `answer_extractor/google_auth.py`
(credentials) and `answer_extractor/google_sheets_export.py`
(duplicate/export-as-PDF/delete against the Drive and Sheets APIs);
`answer_extractor/google_sheets_cli.py` is the standalone setup command
above.

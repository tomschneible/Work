...
def load_sheets(path: str | Path, last_first: bool = False) -> Iterator[Tuple[str, np.ndarray]]:
    """Yield (label, image) pairs for every sheet found at `path`.

    `path` may be a single image file, a single PDF (each page becomes one
    sheet), or a directory containing any mix of the above. `last_first`
    yields a PDF's pages from its last to its first instead; either way a
    page is only rendered when it's asked for, so a caller that stops
    ...
    if path.is_dir():
        for child in sorted(path.iterdir()):
            if child.suffix.lower() in IMAGE_SUFFIXES or child.suffix.lower() in PDF_SUFFIXES:
                yield from load_sheets(child, last_first)
        return
    ...
    if suffix in IMAGE_SUFFIXES:
        ...
        yield from _load_pdf(path, last_first)
    else:
        raise ValueError(f"Unsupported file type: {path}")


def _load_pdf(path: Path, last_first: bool = False) -> Iterator[Tuple[str, np.ndarray]]:
    ...
    try:
        page_indexes = range(len(doc) - 1, -1, -1) if last_first else range(len(doc))
